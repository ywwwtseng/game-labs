...
function useSelectorBridge({
    ...
    (event) => {
      ...
      if (selector.progress && selector.rect.default) {
        ...
        if (isPressRef.current === true) {
          ...
          const pos0 = CanvasUtil.getPosition(
            event,
            document.getElementById(canvasId),
          );
          ...
          if (
            selector.rect.default[0] !== indexX ||
            selector.rect.default[1] !== indexY
          ) {
          ...
          const [x, y, dx, dy] = CanvasUtil.normalizeRect(
        ...
        if (pos.within) {
          if (index[0] >= x && index[0] < x + dx) {
            if (index[1] >= y && index[1] < y + dy) {
              ...
              const pos1 = CanvasUtil.getPosition(
                event,
                document.getElementById(canvasId),
              );
              ...
              const vec = Vec2Util.calc(pos1, {
                sub: Vec2Util.calc(pos0, { add: { x: 1, y: 1 } }),
              });
  ...
  const onMouseUp = useCallback(
    (event) => {
      ...
      const selectedDefaultRect = CanvasUtil.normalizeRect(
        selector.rect.default,
      );
